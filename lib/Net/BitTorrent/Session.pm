...
package Net::BitTorrent::Session;
{
    ...
    our $SVN = q[$Id: Session.pm 29 2008-10-11 15:19:36Z sanko@cpan.org $];
 ...
 #
    ...
    sub hashcheck {
        my ($self) = @_;
        my $no_parent = defined $_client{refaddr $self} ? 1 : 0;
        ...
        for my $index (0 .. ($self->_piece_count - 1)) {
            ...
            if ((($index % 3) == 0) && ($no_parent)) {
                for my $file (@{$files{refaddr $self}}) { $file->_close() }
            }
        }
        ${$status{refaddr $self}} |= 2;
        return 1;
...
=for svn $Id: Session.pm 29 2008-10-11 15:19:36Z sanko@cpan.org $
