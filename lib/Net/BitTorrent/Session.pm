...
use warnings;
{
    ...
    BEGIN {
        ...
        use version qw[qv];
        our $SVN
            = q[$Id: Session.pm 18 2008-04-25 01:14:52Z sanko@cpan.org $];
    ...
    use Net::BitTorrent::Util
        qw[bdecode bencode uncompact compact sum :log];
...
#      get caught with too many open sockets if they pile up
            ...
            {   my @nodes = $self->nodes;
                while (
                     scalar(@nodes)
                     and (
                         scalar(grep { not $_->peer_id } $self->peers)
                         < $self->client->maximum_peers_half_open)
                    )
                {   my $new_peer =
...
=for svn $Id: Session.pm 18 2008-04-25 01:14:52Z sanko@cpan.org $
