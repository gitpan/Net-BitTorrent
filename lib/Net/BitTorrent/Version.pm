...
package Net::BitTorrent::Version;
{
    ...
    use warnings;
    use version qw[qv];
    our $VERSION_BASE = 49; our $UNSTABLE_RELEASE = 9; our $VERSION = sprintf(($UNSTABLE_RELEASE ? q[%.3f_%03d] : q[%.3f]), (version->new(($VERSION_BASE))->numify / 1000), $UNSTABLE_RELEASE);
...
Here, C<NB004S> is the header and C<-rogzGB1v--git> is the trailing
signature.
...
C<S>table or C<U>nstable.  Any deviation from this indicates a bad
...
=item C<NB393S-04.9EraQ--git>

This would be the stable CPAN-ready release C<v0.393>.  The C<--git>
...
Completely legal Peer ID generated by unstable build around C<v65>.
...
=for svn $Id: Version.pm a7a7e9d 2009-02-09 04:49:58Z sanko@cpan.org $
